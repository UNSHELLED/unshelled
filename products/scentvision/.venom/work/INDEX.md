...
## OpenCode project root (critical)

`/venom-spec` and all `.venom/work/...` paths are **relative to the directory OpenCode uses as the project** (`directory` / cwd when you launched `opencode`), **not** “where you think you’re working.”

If you open `opencode` from **repo root** (`venom-mine`), artifacts go to **root** `.venom/work/`. If you want them under **`platforms/unshelled/.venom/`**, launch OpenCode with that folder as the project (e.g. `cd platforms/unshelled` then `opencode`), or open that path in the UI.
