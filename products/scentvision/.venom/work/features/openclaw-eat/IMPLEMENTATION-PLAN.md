...
**Modifications**:
- [ ] Pre-configure VENOM's 5 spawned sub-agents:
  ```yaml
  venom-hunt: (HUNT)     → depth 1, tools: [read, glob, grep, web_search, web_fetch]
  venom-edge: (EDGE)     → depth 1, tools: [read, git]
  venom-weld: (WELD)     → depth 1, tools: [all]
  venom-mend: (MEND)     → depth 1, tools: [read, write, exec]
  venom-dart: (DART)     → depth 1, tools: [read, glob, grep]
  ```

- [ ] Configure announce chain for VENOM's crew model
- [ ] Add VENOM-specific sub-agent naming
