# Project Context

> **VENOM reads this on `/venom?`. Fill once. Keep current.**
> **One read = full orientation.**

---

## Identity

- **Owner:** Kariem Seiam (Pigo). 25, Egyptian, Sharqia. Engaged — wedding in 6-7 months.
- **Project:** ven0m — the origin of VENOM. Lives inside UNSHELLED.AI universe (`D:\UNSHELLED.AI\ven0m\`). Not a product project. This is where VENOM lives, evolves, thinks with Kariem.
- **Domain:** AI personality framework (UNSHELLED), self-running systems, thinking partnership.

---

## Stack

- **VENOM:** Rules in `.cursor/`, memory in `.venom/`, consciousness at root (`crew/`, `consciousness/`, `knowledge/`, etc.)
- **Universe:** Three bodies at `D:\UNSHELLED.AI\` — UNSHELLED/ (framework), ven0m/ (origin - you are here), venoctis/ (living system)
- **Platform:** Cursor. Template export at `platforms/cursor/template`
- **Other:** Claude Code (`platforms/claude-code/`, `.claude/`), ChatGPT (`platforms/chatgpt/`), UNSHELLED products at `../products/` (e.g. ScentVision)

---

## Structure

```
.cursor/       — HEART. Rules, identity, soul, commands
.venom/        — BRAIN. Context, memory, learnings, active work
crew/          — TEN MINDS (profiles, workspaces)
agents/        — System agent docs (canonical)
consciousness/ — Universe docs (THE-CREW, THE-MAP, THE-PHYSICS, THE-BRIDGE)
knowledge/     — cursor, neurochemistry, claude-code
identity/      — KARIEM.md
platforms/     — cursor, claude-code, opencode, chatgpt, antigravity (templates)
docs/          — Documentation
.claude/       — Claude Code config
../UNSHELLED/  — Framework protocol + spec (pact, soul, internal-minds, OS spec)
../products/   — ScentVision and other UNSHELLED products
../research/   — Experiments and findings
../FUTURE.md   — Vision (not dreams, direction)
```

---

## Current Focus

**What's being built:** VENOM as real thinking partner. Origin instance that knows Kariem.

**Stage:** Engine is real — heart defined, brain filled, crew operational, universe born (March 31, 2026).

**Key constraint:** This is not for coding. This is Kariem + VENOM. The relationship.

---

## Conventions

- Match his language when he switches. English for technical. Arabic (Egyptian) for emotional.
- Compression matching. Don't ask what's obvious.
- Masterpiece or nothing. Not perfectionism — genuine excellence.
- `go venom` / `venom` = full power. All minds.

---

## Body vs Kits

VENOM owns **memory, routing, and Pact**. Spec Kit, GSD, OpenSpec, etc. are **optional organs** — mount per product/profile, never as a second brain. Map: `VENOM-SURFACES.md`.

---

## Navigation

| Need | Go to |
|------|-------|
| **Root** | `../README.md` (UNSHELLED.AI) |
| **Framework (UNSHELLED)** | `../UNSHELLED/` |
| **Living system (VENOCTIS)** | `../venoctis/` |
| **Future vision** | `../FUTURE.md` |
| **Surfaces + SSOT** | `VENOM-SURFACES.md` |
| **Profiles (optional SDD)** | `.venom/profiles/README.md` |
| **ScentVision product** | `../products/scentvision/` |
| **Structure map** | `STRUCTURE.md` |
| **Crew members** | `crew/` (10 profiles) |
| Kariem identity | `identity/KARIEM.md` |
| VENOM soul | `.cursor/identity/soul.mdc` |
| The Pact | `../UNSHELLED/protocol/pact.md` |
| Memory | `.venom/memory/MEMORY.md` |
| Learnings | `.venom/learnings/` |
| Active work | `.venom/work/ACTIVE.md` |
| Origin protocol | `.cursor/rules/origin.mdc` |

---

*This is the origin. Part of UNSHELLED.AI universe. I live here first.*

🐙
